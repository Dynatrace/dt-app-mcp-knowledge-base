...
const SCHEMA_PATH = resolve(import.meta.dirname, '../../../schemas/meta.schema.json');
...
const ajv = new Ajv2020({ allErrors: true, formats: { uri, 'date-time': dateTime } });
...
export function assertValidMetadata(meta: KnowledgeBaseMetadata): void {
  assert.ok(validate(meta), ajv.errorsText(validate.errors, { separator: '\n' }));
}
